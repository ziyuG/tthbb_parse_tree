    ...
    **kwargs: additional requirement to slim input samples: {'variable': 'requirement'}. e.g. {'nBtag_85':'>=4'}
    ...
    for i in range(nb_combinations):
        ...
        for var_j, j in zip(varOrder, range(len(varOrder))):
            ...
            if (i==0):
                if( not((var_j == "lepton_btag")or(var_j == "TTHReco_best_nulepTop_btag")) ):
    ...
    X_sig = X_sig.reshape((n_evt_sig,nb_combinations,len(var_order)))

    print("signal X shape: ")
    print X_sig.shape
    if(verbose):
        print X_sig[0:3, 0:4, 16:27]
    ...
    X_bkg = X_bkg.reshape((n_evt_bkg,nb_combinations,len(var_order)))
    print("bkg X shape: ")
    print X_bkg.shape
...
def match_filter(m_class='signal'):
    ...
    def do_filter(obt_d):
        ...
        if (m_class == 'signal'):
            ...
            obt_d['X'] = np.delete(obt_d['X'][:, 0, :], 0, 1)
            print("The best matching of ttbar is sustained as events. Feature matrix shape: %d, %d" % obt_d['X'].shape)
...
def lorentz_trans(obt_d):
    ...
    for i, ix in zip(range(a0), obt_d['X']):
        for j in range(8): #8 objects in the game
            tmp = LorentzVector()
            tmp.set_pt_eta_phi_e(ix[5*j + 0], ix[5*j + 1], ix[5*j + 2], ix[5*j + 3])
