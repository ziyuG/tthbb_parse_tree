...
def kins_trans(x):
    ...
    kins = tf.stack([mag_p, eta, theta, phi, e, pt], 1)
    return kins
...
def m_model():
    ...
    #Wqq
    input_q1w = Input(shape=(4,), name='input_q1w')
    input_q2w = Input(shape=(4,), name='input_q2w')
    ...
    h_lw_merge = keras.layers.concatenate([h_llep, h_rneu, u_lw])
    ...
    model = Model(inputs=[input_q1w, input_q2w, input_bhtop, input_llep, input_rneu, input_bltop, input_b1h, input_b2h], outputs=m_output)
